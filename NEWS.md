...
- In `summarise_distance()` and `variogram()`, `scale_to_km` has similarly been replaced with `coordinate_units` (`"km"` or `"m"`), for consistency with `glgpm()`. Default behaviour is unchanged (`"km"` for `summarise_distance()`, `"m"` for `variogram()`).
